...
require_once("cdash/submission_functions.php");
...
//
function ReleaseProcessingLock($projectid, $mypid)
{
  $unlocked = false;
...
function GetNextSubmission($projectid)
{
  ...
  // Lock the table to avoid a race condition between
  // multiple processes fighting over the same submission.
  $tables = array();
  $tables[] = 'submission';
  ...
  $query_array = pdo_single_row_query(
    "SELECT id, filename, filesize, filemd5sum, attempts FROM submission ".
...
//
function ProcessSubmissionsErrorHandler($projectid, $pid)
{
  ...
  if ($needs_clean_up)
    ...
    //
    if (ReleaseProcessingLock($projectid, $pid))
      {
